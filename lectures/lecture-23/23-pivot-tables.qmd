...
- [It's the process of converting a string into a date]{.alert}
...
:::{style="margin-top: 30px; font-size: 20px;"}
- Last time, we saw how to plot multiple variables, so let's refresh our memory
- We will plot the `sp500` and `dji` columns
...
:::{style="margin-top: 30px; font-size: 20px;"}
- We can plot the differences in both the `sp500` and `djia` columns to see the daily fluctuations
...
plt.plot("date", "diff_djia", data = financial, label = "Dow Jones Industrial Average")
plt.xlabel("Time")
plt.ylabel("Daily Fluctuations")
plt.title("Daily Fluctuations of S&P 500 and Dow Jones Industrial Average")
plt.legend()
plt.show()
```
:::
...
:::{style="margin-top: 30px; font-size: 20px;"}
- As you may have guessed, the plot is still a little messy because of the difference in scales
...
- Highlight regions where there was growth higher than 4\% or below -4\%
...
:::{.columns}
:::{.column width=50%}
- We can also convert the data back to wide format using the `pivot()` function
- The `pivot()` function takes the following arguments:
